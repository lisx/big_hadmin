...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        table.init("${ctx!}/admin/emergency/list?menuType=${folder}",
        function (value, row, index) {
            var operateHtml ='';
            if(row.ifFolder==1){
                operateHtml='<@shiro.hasPermission name="system:train:show"><button class="btn btn-success btn-xs" type="button" onclick="showFolder(\''+row.id+'\',\''+row.fileName+'\')"><i class="fa fa-eye"></i>&nbsp;查看</button> &nbsp;</@shiro.hasPermission>';
            }else{
                operateHtml='<@shiro.hasPermission name="system:train:down"><button class="btn btn-primary btn-xs" type="button" onclick="down(\''+row.id+'\',\''+row.fileName+'\')"><i class="fa fa-download"></i>&nbsp;下载</button> &nbsp;</@shiro.hasPermission>';
            }
    ...
    function onClick(e, treeId, treeNode) {
        ...
        var opt = {
            url: "${ctx!}/admin/emergency/list?menuType=${folder}",
            silent: true,
            query: {
    ...
    //上传资料文件
    function uploadFile() {
        button.uploadFile('${ctx!}/admin/emergency/uploadFile?menuType=${folder}')
    };
    //添加文件夹
    function addFolder() {
        button.addFolder('${ctx!}/admin/emergency/add?menuType=${folder}')
    }
    //下载文件
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="ibox ">
                ...
                <@shiro.hasPermission name="system:train:addFolder">
