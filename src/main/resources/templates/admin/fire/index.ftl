...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        table.init("${ctx!}/admin/emergency/list?menuType=消防安全文件",
            function (value, row, index) {
                var operateHtml ='';
                if(row.ifFolder==1){
                    operateHtml='<@shiro.hasPermission name="system:fire:show"><button class="btn btn-success btn-xs" type="button" onclick="showFolder(\''+row.id+'\',\''+row.fileName+'\')"><i class="fa fa-eye"></i>&nbsp;查看</button>&nbsp;</@shiro.hasPermission>';
                }else{
                    operateHtml='<@shiro.hasPermission name="system:fire:down"><button class="btn btn-primary btn-xs" type="button" onclick="down(\''+row.id+'\',\''+row.fileName+'\')"><i class="fa fa-download"></i>&nbsp;下载</button>&nbsp;</@shiro.hasPermission>';
                }
    ...
    function onClick(e, treeId, treeNode) {
        ...
        var opt = {
            url: "${ctx!}/admin/emergency/list?menuType=消防安全文件",
            silent: true,
            query: {
    ...
    function showFolder(folderId) {
        var url='${ctx!}/admin/fire/toFolder?folderId=' + folderId+'&menuType=消防安全文件';
        button.showFolder(url)
    };
    //上传资料文件
    function uploadFile() {
        var url="${ctx!}/admin/emergency/uploadFile?menuType=消防安全文件";
        button.uploadFile(url)
    };
    //添加文件夹
    function addFolder() {
        button.addFolder('${ctx!}/admin/emergency/add?menuType=消防安全文件')
    }
    //下载文件
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="ibox ">
                ...
                <@shiro.hasPermission name="system:fire:addFolder">
