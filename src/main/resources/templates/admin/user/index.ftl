			...
			$("#table_list").bootstrapTable('destroy').bootstrapTable({
			    ...
			    columns: [{checkbox:true},{
                    ...
                    formatter: function (value, row, index) {
                    	...
                    	operateHtml = operateHtml + '<@shiro.hasPermission name="system:user:role"><button class="btn btn-info btn-xs grantBtn" is-shown="true" type="button" onclick="grant(\''+row.id+'\')"><i class="fa fa-arrows"></i>&nbsp;选择权限</button></@shiro.hasPermission>';
                        return operateHtml;
                    }
			    }]
			});
