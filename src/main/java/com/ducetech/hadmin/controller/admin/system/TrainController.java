...
public class TrainController  extends BaseController {
    ...
    @ResponseBody
    public Page<Train> list() {
        SimpleSpecificationBuilder<Train> builder = new SimpleSpecificationBuilder<>();
        String searchText = request.getParameter("searchText");
        if(!StringUtil.isBlank(searchText)){
            builder.add("name", SpecificationOperator.Operator.likeAll.name(), searchText);
        }
        return trainService.findAll(builder.generateSpecification(), getPageRequest());
    }
        ...
        for (int i =0; i< files.size(); ++i) {
            ...
            if (!file.isEmpty()) {
                try {
                    ...
                    stream.close();
                    train=new Train();
                    train.setFileName(file.getOriginalFilename());
                    train.setFileSize(""+Math.round(file.getSize()/1024));
                    train.setCreateTime(new Date());
                    System.out.println("fileSize"+train.getFileSize());
